    ...
    <div
        ...
        <div class="md:flex flex-row p-3 rounded bg-gray-800">
            <div class="basis-3/4 md:flex justify-start space-x-5">
                ...
                <div class="flex items-center"><p>{{ task.title }}</p></div>
...
export default {
    ...
    setup(){
        const editTask = task => {
            ...
            store.commit('setId', task.id)
            store.commit('setTitle', task.title)
            store.commit('setDeadline', task.deadline)
            store.commit('setEvent', 'editTask')
            store.commit('setBtnName', 'Edit Task')
        }
